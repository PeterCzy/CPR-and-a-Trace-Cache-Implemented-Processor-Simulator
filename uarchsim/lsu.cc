...
void lsu::next_replay_index(bool restart) {
   ...
   if (restart) {
      ...
      scan = MOD_S((unsigned int)(replay_index + 1), lq_size);
      if (scan == lq_tail) {
         scan = lq_head;
      }
      replay_index = -1;
...
void lsu::execute_load(cycle_t cycle,
   ...
   if (stall_disambig) {
      ...
      // utkarsh: Since the load was store forwarded, we need to fix the value of miss_resolve cycle to current value for precise timing
      LQ[lq_index].miss_resolve_cycle = cycle;
   }
...
bool lsu::commit(bool load, bool atomic_op, bool& atomic_success) {
   ...
   cycle_t commit_sequence = load ? proc->PAY.buf[LQ[lq_head].pay_index].sequence : proc->PAY.buf[SQ[sq_head].pay_index].sequence;
   reg_t   commit_pc       = load ? proc->PAY.buf[LQ[lq_head].pay_index].pc : proc->PAY.buf[SQ[sq_head].pay_index].pc;
   ...
   if (load) {
...
#ifdef RISCV_MICRO_DEBUG
      LOG(proc->lsu_log,proc->cycle,commit_sequence,commit_pc,"Committed %s",load ? "load" : "store");
      dump_lq(proc,lq_head,proc->lsu_log);
#endif
      ...
      if(atomic_op){
         atomic_success = true;
         // If the load reservation has been lost, don't send the store to memory
         if(proc->get_state()->load_reservation != SQ[sq_head].addr){
            LOG(proc->lsu_log,proc->cycle,commit_sequence,commit_pc,"Atomic store failed");
            write_store = false;
            atomic_success = false;
         } else {
            LOG(proc->lsu_log,proc->cycle,commit_sequence,commit_pc,"Atomic store succeeded");
         }

      }

      if(write_store){
         // Try to catch the store address exceptions
         //try{
         // Commit store data.
         ...
         {
            unsigned int al_index = proc->PAY.buf[SQ[sq_head].pay_index].AL_index;
            reg_t epc = proc->PAY.buf[SQ[sq_head].pay_index].pc;
            ifprintf(logging_on,proc->lsu_log, "Cycle %" PRIcycle ": core %3d: store exception %s, epc 0x%016" PRIx64 " badvaddr 0x%16" PRIx64 "\n",
                  proc->cycle, proc->id, t.name(), epc, t.get_badvaddr());
            switch(t.cause()){
...
#ifdef RISCV_MICRO_DEBUG
      LOG(proc->lsu_log,proc->cycle,commit_sequence,commit_pc,"Committed %s",load ? "load" : "store");
      dump_sq(proc,sq_head,proc->lsu_log);
#endif
