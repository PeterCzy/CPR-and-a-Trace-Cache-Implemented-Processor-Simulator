...
void pipeline_t::writeback(unsigned int lane_number, size_t& instret) {
//<<<<<<< HEAD
...
//>>>>>>> 4f751980690d6293df8659bec8d053c49b0cd1c7
   ...
   if (Execution_Lanes[lane_number].wb.valid) {
      ...
      if (PAY.buf[index].checkpoint) {
	    ...
	    //printf("next_PC:%d,   c_next_pc:%d\n", PAY.buf[index].next_pc, PAY.buf[index].c_next_pc);
            /*if(!((PAY.buf[index].next_pc == PAY.buf[index].c_next_pc) || !PAY.buf[index].good_instruction)){
            std::cout << (PAY.buf[index].next_pc) << ", " << PAY.buf[index].c_next_pc << ", " << PAY.buf[index].good_instruction << std::endl;
			}*/
            assert((PAY.buf[index].next_pc == PAY.buf[index].c_next_pc) || !PAY.buf[index].good_instruction);
	...
	REN->set_complete(PAY.buf[index].A_valid, PAY.buf[index].A_phys_reg);
	REN->set_complete(PAY.buf[index].B_valid, PAY.buf[index].B_phys_reg);
	REN->set_complete(PAY.buf[index].D_valid, PAY.buf[index].D_phys_reg);
	REN->set_complete_instr_decrement(PAY.buf[index].branch_ID);
      // Tips:
      // 1. At this point of the code, 'index' is the instruction's index into PAY.buf[] (payload).
   ...
   if(REN->precommit_chk()) {
	   //std::cout << cycle << ", " << REN->checkpoint_fifo_head << ", " << REN->checkpoint_fifo[REN->checkpoint_fifo_head].max_instr << std::endl;
      	for (uint64_t lane_number = 0; lane_number < REN->checkpoint_fifo[REN->checkpoint_fifo_head].max_instr; lane_number++) {
          	retire(instret, amo);            // Retire Stage
          	update_timer(&state, instret-prev_instret);
          	prev_instret = instret;
		if (amo){
		//	std::cout << "break: " << lane_number << std::endl;
			break;
		}
     	}
