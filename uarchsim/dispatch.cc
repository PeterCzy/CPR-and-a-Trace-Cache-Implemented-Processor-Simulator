...
//>>>>>>> 4f751980690d6293df8659bec8d053c49b0cd1c7
   ...
   for (i = 0; i < dispatch_width; i++) {
      ...
      //PAY.buf[index].AL_index=REN->dispatch_inst(PAY.buf[index].C_valid,PAY.buf[index].C_log_reg,PAY.buf[index].C_phys_reg,load_flag,store_flag,branch_flag,PAY.buf[index].pc);
      // Tips:
      ...
      if(PAY.buf[index].C_valid){
	   ...
	   // if(PAY.buf[index].C_phys_reg == 281)    
		//printf("ready bit cleared for PR 281 for index %d\n", index);
      }
