...
{
	ifprintf(logging_on,fp, "----------------------------------------------------------\n");
	ifprintf(logging_on,fp, "\tBTB_SIZE   = %d\n", BTB_SIZE);
	ifprintf(logging_on,fp, "\tBTB_MASK   = %x (%d)\n", BTB_MASK, BTB_MASK);
	ifprintf(logging_on,fp, "\tBP_TABLE_SIZE = %d\n", BP_TABLE_SIZE);
	ifprintf(logging_on,fp, "\tBP_INDEX_MASK = %x (%d)\n", BP_INDEX_MASK, BP_INDEX_MASK);
	ifprintf(logging_on,fp, "\tHIST_MASK  = %x (%d)\n", HIST_MASK, HIST_MASK);
	ifprintf(logging_on,fp, "\tHIST_BIT   = %x (%d)\n", HIST_BIT, HIST_BIT);
	ifprintf(logging_on,fp, "\tPC_MASK    = %x (%d)\n", PC_MASK, PC_MASK);
	ifprintf(logging_on,fp, "----------------------------------------------------------\n");
	ifprintf(logging_on,fp, "bpred: ALL CTI\n");
	ifprintf(logging_on,fp, "bpred:     %d / %d = %f\n", stat_num_miss, stat_num_pred,
	        (float)stat_num_miss / (float)stat_num_pred);
	ifprintf(logging_on,fp, "bpred: \n");
	ifprintf(logging_on,fp, "bpred: CONDITIONAL\n");
	ifprintf(logging_on,fp, "bpred:     %d / %d = %f\n", stat_num_cond_miss,
	        stat_num_cond_pred,
	        (float)stat_num_cond_miss /
	        (float)stat_num_cond_pred);
	ifprintf(logging_on,fp, "bpred: \n");
	ifprintf(logging_on,fp, "bpred: RAS flushes = %d\n",stat_num_flush_RAS);
	ifprintf(logging_on,fp, "bpred: \n");
	ifprintf(logging_on,fp, "bpred: conf_corr    = %d (%.3f)\n", stat_num_conf_corr,
	        (double)stat_num_conf_corr / (double)stat_num_pred);
	ifprintf(logging_on,fp, "bpred: conf_ncorr   = %d (%.3f)\n", stat_num_conf_ncorr,
	        (double)stat_num_conf_ncorr / (double)stat_num_pred);
	ifprintf(logging_on,fp, "bpred: nconf_corr   = %d (%.3f)\n", stat_num_nconf_corr,
	        (double)stat_num_nconf_corr / (double)stat_num_pred);
	ifprintf(logging_on,fp, "bpred: nconf_ncorr  = %d (%.3f)\n", stat_num_nconf_ncorr,
